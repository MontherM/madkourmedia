...
function AlbaBuildingModel() {
  ...
  // Track window scroll — map first 700px of scroll to rotation
  const { scrollY } = useScroll()

  // Rotate Y: starts slightly negative (right face visible), turns to left as user scrolls
  const rawRotateY = useTransform(scrollY, [0, 700], [-8, 22])
  const rawRotateX = useTransform(scrollY, [0, 700], [3, -6])
  const rawScale   = useTransform(scrollY, [0, 700], [1, 1.06])

  // Spring smoothing — makes the rotation feel physical, not mechanical
    ...
    >
      ...
      >
        ...
        >
          ...
          <ellipse cx="310" cy="38" rx="48" ry="14" fill="rgba(255,255,255,0.40)" />
          <ellipse cx="350" cy="44" rx="32" ry="10" fill="rgba(255,255,255,0.35)" />
          <ellipse cx="195" cy="24" rx="36" ry="10" fill="rgba(255,255,255,0.30)" />
          ...
          {Array.from({ length: floors }).map((_, f) => {
            ...
            return (
              <g key={f}>
                {/* Glass panel */}
                <rect x={xl+0.5} y={y+slabThick} width={w-1} height={floorH-slabThick-0.5} fill={glassFill} />
                {/* Highlight */}
                <rect x={xl+0.5} y={y+slabThick} width={w*0.32} height={floorH-slabThick-0.5}
                  fill={fi > floors/2 ? "rgba(255,255,255,0.09)" : "rgba(200,220,235,0.07)"} />
                {/* Mullions */}
                {Array.from({ length: 8 }).map((_, m) => (
                  <line key={m}
                    x1={xl+(m+1)*(w/9)} y1={y+slabThick}
                    x2={xl+(m+1)*(w/9)} y2={y+floorH-0.5}
                    stroke="rgba(120,148,168,0.35)" strokeWidth="0.6" />
                ))}
                {/* Slab edge */}
                <rect x={xl-5} y={y} width={w+10} height={slabThick}
                  fill="#CEC9BF" stroke="rgba(175,170,160,0.5)" strokeWidth="0.4" />
                {/* Shadow under slab */}
                <rect x={xl-5} y={y+slabThick} width={w+10} height={2.5} fill="rgba(0,0,0,0.09)" />
              </g>
            )
          })}
              ...
              <g>
                ...
                {[0.12,0.3,0.5,0.7,0.88].map((p,i) => (
                  <rect key={i}
                    ...
                    fill="rgba(195,190,180,0.9)" stroke="rgba(160,155,145,0.3)" strokeWidth="0.4" />
                ))}
                <rect x={xl} y={y+slabThick} width={w} height={floorH-slabThick}
                  fill="rgba(170,195,210,0.28)" />
              </g>
            )
          })()}
          ...
          {/* Roof */}
          <rect
            x={xLeft(floors)-5} y={baseY-floors*floorH-slabThick}
            width={xRight(floors)-xLeft(floors)+10} height={slabThick+2}
            fill="#C4BFB5" stroke="rgba(165,160,150,0.4)" strokeWidth="0.4" />
          ...
          {[
            ...
            <g key={i}>
              <rect x={t.x-3} y={t.y+t.ry-2} width={6} height={baseY-t.y-t.ry+2} fill="rgba(90,70,50,0.35)" />
              <ellipse cx={t.x} cy={t.y} rx={t.rx} ry={t.ry} fill={t.c1} opacity={0.82} />
              <ellipse cx={t.x-t.rx*0.3} cy={t.y-t.ry*0.3} rx={t.rx*0.6} ry={t.ry*0.55} fill={t.c2} opacity={0.5} />
              <ellipse cx={t.x+t.rx*0.25} cy={t.y-t.ry*0.15} rx={t.rx*0.45} ry={t.ry*0.4} fill="rgba(120,160,80,0.25)" />
            </g>
          ))}
          ...
          {/* ALBA HAUS label */}
          <text
            x="200" y={baseY-floors*floorH-30}
            textAnchor="middle"
      ...
      >
        <motion.div
          className="w-px h-6 overflow-hidden relative"
        >
          <motion.div
            className="absolute inset-0"
            style={{ background: "linear-gradient(to bottom, transparent, rgba(27,45,30,0.5), transparent)" }}
            animate={{ y: ["-100%", "100%"] }}
        ...
        <span style={{
          ...
          color:"rgba(27,45,30,0.4)", fontFamily:"var(--font-dm-sans),system-ui"
        }}>
          Scrollen
        </span>
      </motion.div>
