  ...
  return (
    ...
    >
      <nav className="mx-auto flex h-16 max-w-7xl items-center justify-between px-5 sm:px-8">
        <Link href="/academy" className="flex items-center gap-2 font-semibold tracking-tight">
          <span
      ...
      {open && (
        <div className="border-t md:hidden" style={{ borderColor: "var(--ac-border)" }}>
          <div className="flex flex-col gap-1 px-5 py-4">
            ...
            <Link href="/academy/dashboard" onClick={() => setOpen(false)} className="ac-btn ac-btn-ghost mt-2">
              Dashboard
            </Link>
            <Link href="/academy/pricing" onClick={() => setOpen(false)} className="ac-btn ac-btn-primary mt-1">
              Jetzt starten
            </Link>
          </div>
        </div>
