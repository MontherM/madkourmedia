...
export default function CertificateDetail({ id }: { id: string }) {
  ...
  useEffect(() => {
    if (!cert) return
    const verifyUrl = `https://academy.madkourmedia.com/verify/${cert.id}`
  ...
  if (!cert) {
    return (
      <div className="mx-auto max-w-xl px-5 py-24 text-center sm:px-8">
        ...
        <Link href="/certificates" className="ac-btn ac-btn-primary mt-8">
          Zu den Zertifikaten
        </Link>
      </div>
    )
  ...
  return (
    <div className="mx-auto max-w-3xl px-5 py-14 sm:px-8">
      <div className="mb-6 flex items-center justify-between print:hidden">
        <Link href="/certificates" className="text-sm hover:underline" style={{ color: "var(--ac-ink-3)" }}>
          ← Alle Zertifikate
        </Link>
