  ...
  return (
    <div className="mx-auto max-w-7xl px-5 py-8 sm:px-8">
      ...
      <div className="flex flex-wrap items-center gap-2 text-xs" style={{ color: "var(--ac-ink-3)" }}>
        <Link href="/curriculum" className="hover:underline">{levelTitle}</Link>
        <span>/</span>
      ...
      <div className="mt-5 grid gap-8 lg:grid-cols-[1.7fr_1fr]">
        ...
        <div>
              ...
              <ul className="space-y-2">
                ...
                {lesson.resources.map((res) => (
                  ...
                  >
                    ...
                    {res.href ? (
                      <a href={res.href} download className="ac-btn ac-btn-ghost !py-1.5 !px-3 !text-xs">
                        Download
                      </a>
                    ) : (
                      <span className="ac-pill !text-xs">Kommt bald</span>
                    )}
                  </li>
                ))}
              </ul>
          ...
          <div className="mt-4 flex items-center justify-between gap-4 border-t pt-6" style={{ borderColor: "var(--ac-border)" }}>
            {prev ? (
              <Link href={`/lessons/${prev.slug}`} className="ac-btn ac-btn-ghost">← Vorherige</Link>
            ) : <span />}
            {next ? (
              <Link href={`/lessons/${next.slug}`} className="ac-btn ac-btn-primary">Nächste Lektion <ArrowRight width={16} height={16} /></Link>
            ) : <span />}
          </div>
        </div>
        ...
        <aside className="space-y-4 lg:sticky lg:top-20 lg:self-start">
          ...
          {lesson.hasQuiz && (
            <div className="ac-card p-5">
              ...
              <Link href={`/quiz/${lesson.levelId}`} className="ac-btn ac-btn-ghost mt-4 w-full">
                Quiz starten
              </Link>
            </div>
          )}
